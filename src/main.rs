...
fn run() -> Result<(), Error> {
    ...
    output.write_all(now.to_rfc3339_opts(SecondsFormat::Secs, true).as_bytes())?;
...
fn register() -> Result<(), Error> {
    ...
    exec.put_Arguments(&["run-from-task".into()])?;
    let two_mins_from_now = Utc::now() + chrono::Duration::minutes(2);
    let _trigger = def.add_time_trigger(&try_to_bstring!(OsString::from(
        two_mins_from_now.to_rfc3339_opts(SecondsFormat::Secs, true)
    ))?)?;
    def.create(
        &mut folder,
        &try_to_bstring!(OsString::from("S4U test task"))?,
        None,
    )?;
